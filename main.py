...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, ChatMemberAdministrator, ChatMemberOwner
...
async def check_subscription(user_id: int) -> bool:
    try:
        ...
        # Если пользователь не покинул канал и не заблокирован
        if member.status in ["creator", "administrator", "member"]:
            return True
        return False
    except Exception:
        # Если бот не админ в канале или канал не найден/ошибка запроса, пропускаем или считаем по-другому
        return False

async def not_subscribed_message(message: types.Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 Подписаться на канал", url=f"https://t.me/{REQUIRED_CHANNEL.replace('@', '')}")],
        [InlineKeyboardButton(text="🔄 Проверить подписку", callback_data="check_sub")]
    ])
    await message.answer(
        f"❌ Чтобы пользоваться ботом, вы должны быть подписаны на наш канал {REQUIRED_CHANNEL}!\n\nПожалуйста, подпишитесь и нажмите кнопку проверки.",
        reply_markup=kb
    )
...
async def process_check_sub(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Продолжаем регистрацию или запуск
    async with db_pool.acquire() as conn:
        user = await conn.fetchrow("SELECT * FROM users WHERE user_id = $1", callback.from_user.id)
        
    if user and user['photo_id']:
        await callback.message.answer("👋 С возвращением! Вы уже зарегистрированы. Используйте /rate для оценки анкет.")
        return

    await callback.message.answer("👋 Добро пожаловать! Давайте создадим вашу анкету.\n\nСколько вам лет? (Введите число от 13 до 30):")
    await state.set_state(Registration.waiting_for_age)

@dp.message(Command("start"))
async def cmd_start(message: types.Message, state: FSMContext):
    if not await check_subscription(message.from_user.id):
        await not_subscribed_message(message)
        return
...
async def process_age(message: types.Message, state: FSMContext):
    if not message.text.isdigit() or not (13 <= int(message.text) <= 30):
        await message.answer("⚠️ Пожалуйста, введите корректный возраст (число от 13 до 30):")
        return
...
async def cmd_rate(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Пользователь оценивает противоположный пол или любой? Оставляем логику рандома, но смотрим пол оцениваемого, чтобы выдать правильные кнопки
    ...
    # Генерация кнопок в зависимости от пола оцениваемой анкеты
    if target['gender'] == "Женский":
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
            ...
            [
                InlineKeyboardButton(text="⚖️ Ltb", callback_data=f"rate_{target['user_id']}_ltb"),
                InlineKeyboardButton(text="⚖️ Mtb", callback_data=f"rate_{target['user_id']}_mtb")
            ],
            [
                InlineKeyboardButton(text="👍 Htb", callback_data=f"rate_{target['user_id']}_htb"),
                InlineKeyboardButton(text="✨ Stacy-lite", callback_data=f"rate_{target['user_id']}_stacy-lite")
            ],
            [
                InlineKeyboardButton(text="🔥 Stacy", callback_data=f"rate_{target['user_id']}_stacy"),
                InlineKeyboardButton(text="💎 True Eve", callback_data=f"rate_{target['user_id']}_true eve")
            ],
            [
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="⚖️ Ltn", callback_data=f"rate_{target['user_id']}_ltn"),
                InlineKeyboardButton(text="⚖️ Mtn", callback_data=f"rate_{target['user_id']}_mtn")
            ],
            [
                InlineKeyboardButton(text="👍 Htn", callback_data=f"rate_{target['user_id']}_htn"),
                InlineKeyboardButton(text="✨ Chad-lite", callback_data=f"rate_{target['user_id']}_chad-lite")
            ],
            [
                InlineKeyboardButton(text="🔥 Chad", callback_data=f"rate_{target['user_id']}_chad"),
                InlineKeyboardButton(text="💎 True Adam", callback_data=f"rate_{target['user_id']}_true adam")
            ],
            [
                InlineKeyboardButton(text="🚨 Пожаловаться", callback_data=f"report_{target['user_id']}")
            ]
...
async def adm_show_stats(callback: types.CallbackQuery):
    ...
    for row in tier_rows:
        ...
        elif "ltn" in s: tier_counts["ltn"] += 1
        elif "ltb" in s: tier_counts["ltb"] += 1
        elif "mtn" in s: tier_counts["mtn"] += 1
        elif "mtb" in s: tier_counts["mtb"] += 1
        elif "htn" in s: tier_counts["htn"] += 1
        elif "htb" in s: tier_counts["htb"] += 1
        elif "chad-lite" in s: tier_counts["chad-lite"] += 1
        elif "stacy-lite" in s: tier_counts["stacy-lite"] += 1
        elif "chad" in s: tier_counts["chad"] += 1
        elif "stacy" in s: tier_counts["stacy"] += 1
        elif "true adam" in s: tier_counts["true adam"] += 1
        elif "true eve" in s: tier_counts["true eve"] += 1
    ...
    stats_text = (
        ...
        f"• Ltn: **{get_pct('ltn')}** ({tier_counts['ltn']})\n"
        f"• Ltb: **{get_pct('ltb')}** ({tier_counts['ltb']})\n"
        f"• Mtn: **{get_pct('mtn')}** ({tier_counts['mtn']})\n"
        f"• Mtb: **{get_pct('mtb')}** ({tier_counts['mtb']})\n"
        f"• Htn: **{get_pct('htn')}** ({tier_counts['htn']})\n"
        f"• Htb: **{get_pct('htb')}** ({tier_counts['htb']})\n"
        f"• Chad-lite: **{get_pct('chad-lite')}** ({tier_counts['chad-lite']})\n"
        f"• Stacy-lite: **{get_pct('stacy-lite')}** ({tier_counts['stacy-lite']})\n"
        f"• Chad: **{get_pct('chad')}** ({tier_counts['chad']})\n"
        f"• Stacy: **{get_pct('stacy')}** ({tier_counts['stacy']})\n"
        f"• True Adam: **{get_pct('true adam')}** ({tier_counts['true adam']})\n"
        f"• True Eve: **{get_pct('true eve')}** ({tier_counts['true eve']})"
    )

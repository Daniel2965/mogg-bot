...
async def process_photo(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("""INSERT INTO users (user_id, username, age, gender, photo_id, created_at) VALUES ($1, $2, $3, $4, $5, NOW())
            ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username, age = EXCLUDED.age, gender = EXCLUDED.gender, photo_id = EXCLUDED.photo_id""", 
...
async def main_menu_router(message: types.Message):
    ...
    elif "анкета" in text and "изменить" not in text: 
        await show_profile(message)
    elif "изменить анкету" in text:
        await start_edit_profile(message, types.ContextVar("dummy") if False else None) # type: ignore
    elif "топ" in text: 
...
async def start_edit_profile(message: types.Message, state: FSMContext):
    # Если вызывается из хендлера состояний, передаем state
    pass

@dp.message(F.text == "✏️ Изменить анкету")
async def edit_profile_trigger(message: types.Message, state: FSMContext):
    await message.answer("🎂 Укажите ваш новый возраст (от 12 до 30):", reply_markup=types.ReplyKeyboardRemove())
    await state.set_state(EditProfile.waiting_for_age)

@dp.message(EditProfile.waiting_for_age)
async def edit_process_age(message: types.Message, state: FSMContext):
    if not message.text.isdigit() or not (12 <= int(message.text) <= 30):
        await message.answer("⚠️ Введите корректный возраст от 12 до 30:")
        return
    await state.update_data(age=int(message.text))
    kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="👨 Мужчина"), KeyboardButton(text="👩 Женщина")]], resize_keyboard=True)
...
@dp.message(EditProfile.waiting_for_gender)
async def edit_process_gender(message: types.Message, state: FSMContext):
    gender = "male" if "Мужчина" in message.text else "female"
    await state.update_data(gender=gender)
    await message.answer("📸 Отправьте новую фотографию анкеты:", reply_markup=types.ReplyKeyboardRemove())
    await state.set_state(EditProfile.waiting_for_photo)

@dp.message(EditProfile.waiting_for_photo, F.photo)
async def edit_process_photo(message: types.Message, state: FSMContext):
    data = await state.get_data()
    async with db_pool.acquire() as conn:
        await conn.execute("""
            UPDATE users SET age = $1, gender = $2, photo_id = $3 WHERE user_id = $4
        """, data['age'], data['gender'], message.photo[-1].file_id, message.from_user.id)
    await state.clear()
...
async def start_rating(message: types.Message):
    ...
    buttons = []
    # Формируем сетку кнопок 2 в ряд, как на предоставленном скриншоте
    for i in range(0, len(scores), 2):
        row = [InlineKeyboardButton(text=scores[i][0], callback_data=f"set_score_{target['user_id']}_{scores[i][1]}")]
        if i + 1 < len(scores):
            row.append(InlineKeyboardButton(text=scores[i+1][0], callback_data=f"set_score_{target['user_id']}_{scores[i+1][1]}"))
        buttons.append(row)
    
    # Кнопки Связаться и Жалоба точно как на дизайне
    action_row = []
    if target['username']:
        action_row.append(InlineKeyboardButton(text="💬 Хочу пообщаться", url=f"https://t.me/{target['username']}"))
    buttons.append(action_row)
    buttons.append([InlineKeyboardButton(text="🚩 Жалоба", callback_data=f"report_{target['user_id']}")])

    kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    
    uname_display = f"@{target['username']}" if target['username'] else f"ID:{target['user_id']}"
    caption = f"🕸️ {uname_display}, {target['age']} · Средний тир: {rating}\n👥 Анкету оценили: {count}\n\nКакой тир поставишь?"
...
async def rate_target_callback(callback: types.CallbackQuery):
    # Совместимость со старыми вызовами, если есть
    await callback.answer()
...
async def save_score_callback(callback: types.CallbackQuery):
    ...
    score = "_".join(parts[3:])
    voter_id = callback.from_user.id

    async with db_pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO ratings (voter_id, target_id, score, created_at) VALUES ($1, $2, $3, NOW())
            ON CONFLICT (voter_id, target_id) DO UPDATE SET score = EXCLUDED.score, created_at = NOW()
...
async def process_report_reason(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO reports (target_id, voter_id, reason, created_at) VALUES ($1, $2, $3, NOW())", 
            target_id, voter_id, reason
        )
        target = await conn.fetchrow("SELECT photo_id, username FROM users WHERE user_id = $1", target_id)

    await message.answer("🚩 Жалоба отправлена администратору! Спасибо за сигнал.")
    
    if ADMIN_ID_NUM and target:
        caption = (
            f"🚨 **НОВАЯ ЖАЛОБА!**\n\n"
            f"👤 На кого: @{target['username'] or 'Без юзернейма'} (`{target_id}`)\n"
            f"🕵️ От кого: @{message.from_user.username or 'Без юзернейма'} (`{voter_id}`)\n"
            f"💬 **Причина:** {reason}"
        )
        try:
            if target['photo_id']:
                await bot.send_photo(chat_id=ADMIN_ID_NUM, photo=target['photo_id'], caption=caption, parse_mode="Markdown")
            else:
                await bot.send_message(chat_id=ADMIN_ID_NUM, text=caption, parse_mode="Markdown")
        except Exception as e:
            logging.error(f"Не удалось отправить жалобу админу: {e}")
...
async def get_user_calculated_rating(user_id: int, gender: str):
    ...
    scores_list = [s[1] for s in (MALE_SCORES if gender == 'male' else FEMALE_SCORES)]
    ...
    # Красивое отображение тира с заглавной буквы
    return best_score.upper(), len(rows)
...
async def show_tops(message: types.Message):
    ...
    for u in users:
        ...
        scores_list = [s[1] for s in (MALE_SCORES if u['gender'] == 'male' else FEMALE_SCORES)]
        rank = scores_list.index(rating.lower()) if rating.lower() in scores_list else -1
...
async def adm_show_stats(callback: types.CallbackQuery):
    ...
    for row in tier_rows:
        s = row['score'].lower()
        if s in ["sub 3"]: tier_counts["sub 3"] += 1
        elif s in ["sub 5"]: tier_counts["sub 5"] += 1
        elif s in ["ltn", "ltb"]: tier_counts["ltn/ltb"] += 1
        elif s in ["mtn", "mtb"]: tier_counts["mtn/mtb"] += 1
        elif s in ["htn", "htb"]: tier_counts["htn/htb"] += 1
        elif s in ["chad-lite", "stacy-lite"]: tier_counts["chad-lite/stacy-lite"] += 1
        elif s in ["chad", "stacy"]: tier_counts["chad/stacy"] += 1
        elif s in ["adam-lite", "true adam", "eve-lite", "true eve"]: tier_counts["true adam/true eve"] += 1
    ...
    stats_text = (
        ...
        f"📝 Количество созданных анкет за день: **{new_users}**\n"
        f"🚨 Количество жалоб за день: **{new_reports}**\n"
        f"✅ Количество принятых жалоб за день: **{accepted_reports}**\n"
        f"🔥 Количество оцененных анкет за день: **{ratings_count}**\n\n"
        f"📈 **Распределение тиров за день:**\n"
        f"• Sub 3: **{get_pct('sub 3')}** ({tier_counts['sub 3']})\n"
        f"• Sub 5: **{get_pct('sub 5')}** ({tier_counts['sub 5']})\n"
        f"• Ltn/Ltb: **{get_pct('ltn/ltb')}** ({tier_counts['ltn/ltb']})\n"
        f"• Mtn/Mtb: **{get_pct('mtn/mtb')}** ({tier_counts['mtn/mtb']})\n"
        f"• Htn/Htb: **{get_pct('htn/htb')}** ({tier_counts['htn/htb']})\n"
        f"• Chad-lite/Stacy-lite: **{get_pct('chad-lite/stacy-lite')}** ({tier_counts['chad-lite/stacy-lite']})\n"
        f"• Chad/Stacy: **{get_pct('chad/stacy')}** ({tier_counts['chad/stacy']})\n"
        f"• True Adam/True Eve: **{get_pct('true adam/true eve')}** ({tier_counts['true adam/true eve']})"
    )

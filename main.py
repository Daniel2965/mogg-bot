...
# Нормализованные ключи для статистики
TIER_KEYS = ["sub 3", "sub 5", "ltn/ltb", "mtn/mtb", "htn/htb", "chad-lite/stacy-lite", "chad/stacy", "true adam/true eve"]
...
async def rate_target_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        target = await conn.fetchrow("SELECT gender, username FROM users WHERE user_id = $1", target_id)
    if not target:
        await callback.answer("Анкета не найдена")
        return
    
    scores = MALE_SCORES if target['gender'] == 'male' else FEMALE_SCORES
    buttons = [[InlineKeyboardButton(text=score, callback_data=f"set_score_{target_id}_{score}")] for score in scores]
    
    # Добавление кнопок Связаться и Пожаловаться рядом с тирами
    action_row = []
    if target['username']:
        action_row.append(InlineKeyboardButton(text="💬 Связаться", url=f"https://t.me/{target['username']}"))
    action_row.append(InlineKeyboardButton(text="🚨 Пожаловаться", callback_data=f"report_{target_id}"))
...
async def process_report_reason(message: types.Message, state: FSMContext):
    ...
    # Отправка фото нарушителя с причиной админу
